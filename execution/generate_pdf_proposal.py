...
def send_proposal_email(
    ...
    """Dispatches the generated PDF proposal as an email attachment to the recipient (supporting directed recipients and CC)."""
    import smtplib
    ...
    if requester_email and requester_email.lower() != recipient_email.lower():
        intro_greeting = f"Hi Team {org_name},\n\nThank you for connecting with us! As requested by {requester_email}, attached is the official 1-page Digital Elevation & Growth Proposal for {org_name} prepared by {sender_name} & Team."
    else:
    ...
    meeting_callout = f"\n• Schedule a 10-Min Walkthrough Call: {calendar_url}\n" if calendar_url else ""

    body_text = f"""{intro_greeting}
...
• Milestone Schedule & 50% Deposit Schedule
{meeting_callout}
Please review the attached PDF document. Whenever you are ready to initiate Phase 1 Onboarding or have any questions:
👉 Book a brief call: {calendar_url if calendar_url else 'Reply to this email'}
👉 Or message directly on WhatsApp: https://wa.me/{my_wa} (+{my_wa})
...
"""
    ...
    try:
        ...
        print(f"[+] SUCCESS! PDF Proposal emailed directly to {recipient_email}{cc_info}")
        return True
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cc-email", type=str, default="", help="Optional CC email address")
    parser.add_argument("--requester-email", type=str, default="", help="Original requesting email address if directed")
    ...
    if args.send_email and pdf_file:
        ...
        if not target_email and TMP_CSV.exists():
            ...
            if not matched.empty:
                # Check directed email first
                target_email = str(matched.iloc[0].get("directed_proposal_email", "")).strip()
        ...
        if target_email and "@" in target_email:
            send_proposal_email(
                ...
                org_name=args.lead_name,
                pdf_path=pdf_file,
                cc_email=args.cc_email if args.cc_email else None,
                requester_email=args.requester_email if args.requester_email else None
            )
        else:
