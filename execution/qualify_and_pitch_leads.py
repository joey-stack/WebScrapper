#!/usr/bin/env python3
"""
Layer 3: Execution Tool - Principal Lead Qualification, ICP Scoring, Case Study Proof & AI Pitch Generator
Part 1 & Part 3 of Jordan Platten's 5-Part Client Acquisition Engine:
- Lead ICP Scoring Engine (0-100 Points) & Priority Tiering (Tier A / B / C)
- Non-contradictory, factual profile highlight & improvement area extraction
- Industry Case-Study Proof Point Injection
- Factual Elevation Roadmap & 5-Star ELI5 Conversion Strategy
- Multi-channel Surround Sound WhatsApp outreach copy generator
- Strict rating vs review count consistency
"""
...
def qualify_lead(row: dict) -> str:
    ...
    try:
        reviews_count = int(re.sub(r"[^\d]", "", str(row.get("reviews_count", 0) or "0")) or 0)
    except Exception:
        reviews_count = 0
...
def calculate_icp_score(row: dict) -> tuple:
    """
    Jordan Platten ICP Scoring Engine (0-100 Points):
    - Missing Website: +30 pts (High conversion urgency)
    - 0 Reviews: +20 pts (Critical social proof deficiency)
    - 1 to 4 Reviews: +15 pts (Under-leveraged social proof)
    - Buried Rank (#11 to #30): +20 pts (Prime opportunity zone)
    - Rank (#31 to #50+): +10 pts (Deep visibility gap)
    - Missing Phone: +10 pts (Immediate contact optimization)
    - High-Ticket Industry Tier: +20 pts (NGOs/Foundations, Tech/Software, Real Estate, Healthcare, Private Schools)
    """
    score = 0
    website = str(row.get("website", "") or "").strip()
    phone = str(row.get("phone", "") or "").strip()
    ...
    # 2. Reviews status (Max 20 pts)
    if reviews_count == 0:
        score += 20
    elif reviews_count < 5:
        score += 15

    # 3. Rank Position (Max 20 pts)
    if 11 <= rank_pos <= 30:
        score += 20
    elif rank_pos > 30:
        score += 10
    ...
    # 4. Missing Phone (Max 10 pts)
    if not phone:
        score += 10

    # 5. High-Ticket / High-Budget Sector (Max 20 pts)
    high_ticket_keywords = [
        "ngo", "foundation", "non-profit", "charity", "trust",
        "software", "tech", "technology", "app", "saas",
        "real estate", "property", "developer", "estate", "realtor",
        "hospital", "clinic", "dental", "medical", "health",
        "school", "college", "academy", "institute", "university"
    ]
    if any(kw in category or kw in name for kw in high_ticket_keywords):
        score += 20
    else:
        score += 10
    ...
    if score >= 80:
        tier = "Tier A: High-Intent Hot Lead"
    elif score >= 60:
        tier = "Tier B: High-Potential Prospect"
    else:
        tier = "Tier C: Secondary Opportunity"

    return score, tier

def generate_industry_proof_point(category: str, location: str) -> str:
    """Injects localized, sector-specific real-world proof to boost conversion."""
    cat_lower = category.lower() if category else ""
    if any(kw in cat_lower for kw in ["ngo", "foundation", "non-profit", "charity"]):
        return f"In {location}, verified non-profits with modern audit portals capture over 40% more institutional grant and partner funding."
    elif any(kw in cat_lower for kw in ["tech", "software", "saas", "app"]):
        return f"Tech companies in {location} with optimized web conversion and Top 3 search visibility capture 3x more demo requests."
    elif any(kw in cat_lower for kw in ["real estate", "property", "developer"]):
        return f"Top real estate agencies in {location} capturing Top 3 Map Pack rankings average 60%+ more direct buyer inquiries from diaspora clients."
    elif any(kw in cat_lower for kw in ["hospital", "clinic", "medical", "dental"]):
        return f"Private clinics in {location} with complete Google Business verification and 10+ reviews see a 50%+ rise in appointment calls."
    elif any(kw in cat_lower for kw in ["school", "college", "academy", "education"]):
        return f"Educational institutions in {location} with verified web profiles see a 45% increase in seasonal admission inquiries."
    else:
        return f"Organizations in {location} ranking in the Top 3 Map Pack capture over 44% of all local customer search clicks."
...
def generate_profile_highlight(row: dict) -> str:
    """Extracts an accurate, factual positive highlight grounded in real GMB data without contradictions."""
    ...
    try:
        reviews_cnt = int(re.sub(r"[^\d]", "", str(row.get("reviews_count", 0) or "0")) or 0)
    except Exception:
        reviews_cnt = 0

    try:
        rating_val = float(rating_str) if rating_str and rating_str != "N/A" else 0.0
    except Exception:
        rating_val = 0.0

    highlights = []

    # ONLY praise rating if reviews_cnt >= 1 and rating_val >= 4.0
    if reviews_cnt >= 5 and rating_val >= 4.0:
        highlights.append(f"strong community trust ({rating_val}★ review score across {reviews_cnt} Google reviews)")
    elif reviews_cnt >= 1 and rating_val >= 4.0:
        highlights.append(f"positive feedback ({rating_val}★ rating on Google Maps)")
    elif reviews_cnt >= 1:
        highlights.append(f"community feedback across {reviews_cnt} public review{'s' if reviews_cnt > 1 else ''}")

    # Category context
    if category and category.lower() not in ["ngo", "non-profit organization", "business", "establishment"]:
        clean_cat = category.strip()
        highlights.append(f"active mission as a {clean_cat}")

    # Location context
    if "abuja" in address.lower() or "abuja" in name.lower():
        highlights.append("established presence in Abuja")
    elif "lagos" in address.lower() or "lagos" in name.lower():
        highlights.append("established presence in Lagos")
    elif address:
        loc_match = re.search(r"([A-Za-z\s]+),\s*Nigeria", address, re.IGNORECASE)
        if loc_match:
            highlights.append(f"established presence in {loc_match.group(1).strip()}")
        else:
            highlights.append("established local community presence")
    else:
        highlights.append("established organizational presence")

    # Combine at most 2 specific highlights
    if len(highlights) > 2:
        highlights = highlights[:2]

    return " & ".join(highlights) if highlights else "established organizational presence"
...
def generate_principal_conversion_strategy(row: dict, service_needed: str, highlight: str) -> str:
    ...
    if service_needed == "Website Design Only":
        strategy = (
            ...
            f"• Primary Problem Identified: Missing official web portal (invisible for institutional partner due diligence & compliance verification).\n\n"
            f"1. THE REAL-WORLD ANALOGY (Simple Explanation):\n"
            f"   Think of a website like your organization's 'Digital Glass Headquarters & Reception Desk'. Right now, when major international partners or committees look for you online, there is no building there—just an empty field. Building a modern web portal is like putting up a clean, welcoming glass building in {location} with a 24/7 receptionist taking inquiries.\n\n"
            ...
            f"   • Step 2 (Automated Intake Funnel): Integrate frictionless intake forms with automated email receipts for partnership requests.\n"
            f"   • Step 3 (Secure Multi-Currency Gateway): Connect secure payment channels (NGN, USD, EUR, GBP) for recurring donor/client support.\n"
        ...
        strategy = (
            ...
            f"   • Step 2 (Category & Local Keyword Tuning): Align primary & secondary GMB categories and inject high-intent localized search terms.\n"
            f"   • Step 3 (Geotagged Proof Uploads): Publish 20+ geotagged activity photos showing real community impact in action.\n"
            f"   • Step 4 (Compliant Review Engine): Deploy an authentic feedback collection workflow to generate verified reviews, propelling {name} into the Top 3 Map Pack.\n\n"
            f"3. EXPECTED OUTCOME & TIMELINE:\n"
        ...
        strategy = (
            ...
            f"   • Step 2 (Glass Headquarters Front Door): Build a fast, mobile-first web portal linked directly to your map profile with intake forms and compliance reports.\n"
            f"   • Step 3 (Review & Reputation System): Launch a compliant review request workflow to build authentic proof on Google Maps.\n"
...
def generate_improvement_area(row: dict) -> str:
    """Extracts concrete, factual areas where the profile needs improvement without contradiction."""
    website = str(row.get("website", "") or "").strip()
    phone = str(row.get("phone", "") or "").strip()
    address = str(row.get("address", "") or "").strip()
    ...
    if rank_pos > 3:
        if rank_pos <= 10:
            ...
            deficiencies.append(f"your listing is currently ranked at Position #{rank_pos} on Google Maps, placing it on secondary pages where over 80% of online searchers never scroll")
            
    # 4. Missing contact info
    if not phone:
        deficiencies.append("your listing lacks a direct clickable phone contact number for instant callback inquiries")

    if deficiencies:
        selected = deficiencies[:2]
        if len(selected) == 1:
            return f"Specifically, {selected[0]}."
        else:
            return f"Specifically, {selected[0]}, and {selected[1]}."
    else:
        return "Specifically, your profile has key opportunities to refine category keywords and build local directory citation authority."
...
def generate_consultative_pitch(row: dict, service_needed: str, highlight: str) -> str:
    ...
    Explicitly highlights complimentary strengths, industry case study proof, and specific profile improvement areas without contradiction.
    """
    raw_name = str(row.get("name", "Organization")).strip()
    name = get_clean_name(raw_name)
    category = str(row.get("category", "") or "").strip()
    ...
    if not category or category.lower() in ["ngo", "non-profit organization", "business", "establishment"]:
        category_noun = "organizations"
    else:
        category_noun = f"{category.lower()}s" if not category.lower().endswith("s") else category.lower()

    improvement_note = generate_improvement_area(row)
    proof_point = generate_industry_proof_point(category, location)

    if service_needed == "Website Design Only":
        pitch = (
            f"Hello Team {name},\n\n"
            f"I wanted to commend your team's work and {highlight}.\n\n"
            f"During our digital review of {category_noun} in {location}, we noticed a key growth area: {improvement_note}\n\n"
            f"{proof_point}\n\n"
            f"Establishing an official, modern web portal with integrated intake/inquiry forms, program portfolios, and verified compliance credentials will allow prospective partners and committees to easily verify your work and scale your impact.\n\n"
            f"We specialize in building modern, high-performing web platforms for {category_noun} in {location}. "
            f"Would you be open to a brief 5-minute consultative call this week to review a complimentary website concept tailored specifically for {name}?"
        )
    elif service_needed == "GMB SEO Optimization Only":
        pitch = (
            f"Hello Team {name},\n\n"
            f"I was recently researching {category_noun} in {location} and was impressed by {name}'s {highlight}.\n\n"
            f"During our local search review, we identified an immediate area for improvement: {improvement_note}\n\n"
            f"{proof_point}\n\n"
            f"With local clients and partners turning to Google Search daily, optimizing your profile completeness, category keywords, and Google Maps prominence presents a major opportunity to capture steady organic search inquiries over the next 3 to 6 months.\n\n"
            f"Would you have 5 minutes this week for a short introductory call to discuss practical steps for elevating {name}'s local Google presence?"
        )
    elif service_needed == "Both (Website + GMB SEO)":
        pitch = (
            f"Hello Team {name},\n\n"
            f"I am reaching out to commend {name}'s ongoing work and {highlight}.\n\n"
            f"In our digital audit of {category_noun} in {location}, we identified key areas where your digital visibility can be significantly improved: {improvement_note}\n\n"
            f"{proof_point}\n\n"
            ...
            f"We build practical web and local search strategies tailored for organizations across {location}. "
            f"Would you have 5 minutes this week for a brief call to review a tailored digital roadmap for {name}?"
        )
    else:
        pitch = (
            f"Hello Team {name},\n\n"
            ...
            f"{proof_point}\n\n"
            f"We assist established {category_noun} in optimizing their web portals and maintaining AI search engine indexing (GEO). "
...
def generate_variant_pitch_and_subject(row: dict, service_needed: str, highlight: str, variant_id: str = None) -> tuple:
    """Generates pitch copy and subject line tailored to an assigned evolutionary variant."""
    try:
        from optimize_pitch_performance import load_ledger, select_optimal_variant_id
        ledger = load_ledger()
        if not variant_id or variant_id not in ledger.get("variants", {}):
            variant_id = select_optimal_variant_id()
        vdata = ledger["variants"].get(variant_id, {})
    except Exception:
        variant_id = "AUTHORITY_V1"
        vdata = {}
    ...
    clean_cat = category.lower().replace("ngo", "non-profit") if category else "local business"
    
    try:
        rank_val = int(row.get("search_rank_position", row.get("rank_position", 12)))
    except Exception:
        rank_val = 12
    rank_str = f"Position #{rank_val}" if rank_val else "outside the Top 3"
    ...
    subjects = vdata.get("subject_templates", [
        ...
        "1-page roadmap for {name} in {location}",
        "Quick question regarding {name}'s search presence"
    ])
    raw_subject = random.choice(subjects) if subjects else "Quick note for {name}"
    formatted_subject = raw_subject.format(
        name=clean_name,
        location=location,
        category=clean_cat,
        rank_str=rank_str,
    ...
    template = vdata.get("pitch_template", "")
    if template:
        formatted_pitch = template.format(
            name=clean_name,
            location=location,
            highlight=highlight,
            clean_cat=clean_cat,
        ...
        formatted_pitch = generate_consultative_pitch(row, service_needed, highlight)

    return variant_id, formatted_subject, clean_duplicate_locations(formatted_pitch)
...
def generate_whatsapp_outreach_copy(row: dict, service_needed: str, highlight: str) -> str:
    """Generates ready-to-send 1-click WhatsApp conversational copy for multi-channel Day 2 touch."""
    raw_name = str(row.get("name", "Team")).strip()
    name = get_clean_name(raw_name)
    location = "Nigeria"
    if "abuja" in str(row.get("address", "")).lower() or "abuja" in raw_name.lower():
        location = "Abuja"
    ...
    if "Website" in service_needed and "GMB" in service_needed:
        core_offer = f"a custom web portal and Google Maps Top 3 optimization for {name}"
    elif "Website" in service_needed:
        core_offer = f"a modern, high-converting official web portal for {name}"
    else:
        core_offer = f"elevating {name}'s Google Maps listing into the Top 3 Map Pack in {location}"

    msg = (
        f"Hi Team {name},\n\n"
        f"I sent a quick email earlier regarding {highlight} in {location}. "
        f"We put together a tailored concept for {core_offer} to help you capture more inbound inquiries and partner credibility.\n\n"
        f"Would you be open to a quick 3-minute chat or reviewing a complimentary 1-page roadmap? Let me know here on WhatsApp (+{my_wa})!"
    )
    return msg

def process_and_qualify_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """
    Processes and enriches DataFrame with:
    - ICP Score (0-100) and Priority Tier (Tier A, B, C)
    - Service Needed, Price Class, Pricing, Scope Breakdown
    - Non-contradictory Profile Highlights & Elevation Proposals
    - 5-Star Conversion Strategy with ELI5 Analogy
    - Personalized Consultative Pitch with Industry Case Study Proof
    - Multi-Channel WhatsApp Outreach Copy & 1-Click Link
    - Sorted by ICP Score Descending (Highest-ROI leads first)
    """
    icp_scores = []
    priority_tiers = []
    services = []
    ...
    for idx, row in df.iterrows():
        ...
        # 1. Lead Qualification & ICP Scoring
        svc = qualify_lead(row_dict)
        score, tier = calculate_icp_score(row_dict)
        ...
        # Evolutionary A/B Variant Assignment
        variant_id, subj, ptch = generate_variant_pitch_and_subject(row_dict, svc, hlt)
        
        category = str(row_dict.get("category", "") or "").strip()
        location = "Abuja" if "abuja" in str(row_dict.get("address", "")).lower() else ("Lagos" if "lagos" in str(row_dict.get("address", "")).lower() else "Nigeria")
        proof = generate_industry_proof_point(category, location)
        
        wa_url = generate_whatsapp_link(str(row_dict.get("phone", "")), str(row_dict.get("name", "")))
        wa_msg = generate_whatsapp_outreach_copy(row_dict, svc, hlt)
        ...
        wa_copies.append(wa_msg)
        pitch_variants.append(variant_id)
    ...
    df["industry_proof_point"] = proof_points
    df["organization_elevation"] = elevations
    df["conversion_strategy"] = strategies
    df["pitch_variant_id"] = pitch_variants
    df["email_subject"] = email_subjects
    df["personalized_pitch"] = pitches
    df["whatsapp_outreach_copy"] = wa_copies
    df["whatsapp_chat_link"] = wa_links

    # Filter to keep buried leads (Rank #11 to #50+)
    df_filtered = df[
        (df["search_rank_position"] > 10)
    ].copy().reset_index(drop=True)

    # Sort by ICP Score descending so highest-converting Tier A leads are processed first
    df_filtered = df_filtered.sort_values(by="icp_score", ascending=False).reset_index(drop=True)
    ...
    column_order = [
        ...
        "industry_proof_point", "organization_elevation", "conversion_strategy", "personalized_pitch", 
...
if __name__ == "__main__":
    print("Principal Lead Qualification, ICP Scoring (0-100) & Jordan Platten 5-Part Engine Generator Ready.")
